...
<head>
    ...
    <style>
        ...
        .repair-table thead th:first-child { border-top-left-radius: 10px; border-bottom-left-radius: 10px; }
        .repair-table thead th:last-child { border-top-right-radius: 10px; border-bottom-right-radius: 10px; }
        ...
        .repair-table tbody tr { transition: transform 0.2s ease-in-out, box-shadow 0.2s ease-in-out; }
        .repair-table tbody tr:hover { transform: translateY(-3px); box-shadow: 0 6px 15px rgba(0,0,0,0.1); z-index: 2; }
        .repair-table tbody tr td:first-child { border-top-left-radius: 8px; border-bottom-left-radius: 8px; }
        .repair-table tbody tr td:last-child { border-top-right-radius: 8px; border-bottom-right-radius: 8px; }
        .repair-table th.text-center, .repair-table td.text-center { text-align: center; }
        ...
        .status-badge.success { background-color: #e6ffed; color: #28a745; border: 1px solid #28a745; }
        .status-badge.pending { background-color: #fff8e1; color: #ffc107; border: 1px solid #ffc107; }
        .status-badge.in-progress { background-color: #e0f2f7; color: #17a2b8; border: 1px solid #17a2b8; }
        .status-badge.failed { background-color: #ffe6e6; color: #dc3545; border: 1px solid #dc3545; }

        .type-tag { background-color: #f0f4f7; color: #6c757d; padding: 5px 10px; border-radius: 5px; font-size: 12px; font-weight: 500; display: inline-block; white-space: nowrap; }

        .action-cell { display: flex; align-items: center; gap: 10px; justify-content: center; }
        .btn-action { padding: 8px 15px; font-size: 13px; border-radius: 8px; display: inline-flex; align-items: center; gap: 8px; font-weight: 500; border: none; box-shadow: 0 2px 5px rgba(0,0,0,0.05); white-space: nowrap; }
        .btn-action:hover { transform: translateY(-2px); box-shadow: 0 4px 10px rgba(0,0,0,0.1); }
        .btn-action.btn-primary { background-color: #007bff; color: #fff; }
        .btn-action.btn-success { background-color: #28a745; color: #fff; }
        
        .datetime-display { display: flex; flex-direction: column; align-items: center; font-size: 14px; white-space: nowrap; }
    ...
    <table class="repair-table">
        <thead>
            <tr>
                ...
                <th><i class="fas fa-file-alt"></i> รายละเอียดแจ้งซ่อม</th> 
                <th class="text-center"><i class="far fa-calendar-alt"></i> วันที่แจ้ง</th>
                <th class="text-center"><i class="fas fa-check-calendar"></i> วันที่ซ่อมเสร็จ</th>
                <th class="text-center"><i class="fas fa-info-circle"></i> สถานะ</th>
            ...
            if ($result->num_rows > 0) {
                while($row = $result->fetch_assoc()):
                    ...
                    switch($row['status']){
                        ...
                        case 'รอดำเนินการ': $status_class='pending'; $status_icon='<i class="fas fa-hourglass-half"></i>'; break;
                        case 'กำลังซ่อม': $status_class='in-progress'; $status_icon='<i class="fas fa-tools"></i>'; break;
                        case 'ซ่อมไม่สำเร็จ': $status_class='failed'; $status_icon='<i class="fas fa-exclamation-triangle"></i>'; break;
                    }
